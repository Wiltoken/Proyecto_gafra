...
<head>
    ...
    <title><?php echo isset($data["titulo"]) ? $data["titulo"] : "Operarios"; ?></title>
...
<aside id="lateral">
        ...
        <section class="layout">
            ...
            <div class="table-container">
                <div class="tabla-botones">
                    <button onclick="window.location.href='index.php?controller=usuarios&action=nuevo'" class="tabla-btn">Nuevo</button>
                    <button onclick="modificarUsuario()" class="tabla-btn">Editar</button>
                    <button onclick="eliminarUsuario()" class="tabla-btn">Borrar</button>
                </div>
                    ...
                    <tbody>
                        <?php if (isset($data["usuarios"]) && is_array($data["usuarios"])): ?>
                            <?php foreach ($data["usuarios"] as $usuario): ?>
                                <tr>
                                    <td><input type="checkbox" class="row-checkbox" data-user-id="<?php echo $usuario['id']; ?>"></td>
                                    <td><?php echo $usuario['id']; ?></td>
                                    <td><?php echo $usuario['nombre']; ?></td>
                                    <td><?php echo $usuario['apellido']; ?></td>
                                    <td><?php echo $usuario['correo']; ?></td>
                                    <td><?php echo $usuario['clave_acceso']; ?></td>
    ...
    // Función para redireccionar a la página de  
    function modificarUsuario() {
        ...
        if (checkedCount === 1) {
            window.location.href = 'index.php?controller=usuarios&action=actualizar&id=' + userId;
        } else if (checkedCount > 1) {
            alert('Solo puedes modificar un usuario a la vez');
        } else {
    ...
    function eliminarUsuario() {
        ...
        if (checkedUserIds.length > 0) {
            ...
            if (confirmation) {
                // Redirigir al controlador para eliminar los usuarios
                window.location.href = 'index.php?controller=usuarios&action=eliminar&ids=' + checkedUserIds.join(',');
            }
        } else {
